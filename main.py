...
def validate_input(request: TranslationRequest):
    try:
        ...
        class TextValidator(PandasDataset):
            pass

        validator = TextValidator(df)

        # Add expectations
        validator.expect_column_values_to_not_be_null("text")
        validator.expect_column_value_lengths_to_be_between("text", min_value=3, max_value=5000)
        validator.expect_column_values_to_match_regex("text", r"^[A-Za-z0-9\s.,!?@'\"()]+$")
        ...
        if result["success"]:
            ...
            return {"valid": False, "errors": result["results"]}
